...
##################################################################
#
# Processing Script for Suspension-Rates-by-Gender
...
names(susp_rates_state)[names(susp_rates_state)=="State"] <- "District"
...
write.table(
  ...
  file.path(path_to_top_level, "data", "suspension_rates_gender_2020.csv"),
  sep = ",",
  row.names = F
)
